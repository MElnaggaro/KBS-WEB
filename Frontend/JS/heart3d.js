...
loader.load(
	...
	(gltf) => {
		...
		window._heartGroup.position.set(
			initial.position.x,
			initial.position.y,
			initial.position.z,
		);
		window._heartGroup.rotation.set(
			initial.rotation.x,
			initial.rotation.y,
			initial.rotation.z,
		);
...
function animate() {
	...
	heartState.vibration +=
		(heartTarget.vibration - heartState.vibration) * LERP_SPEED;
	heartState.glowLightIntensity +=
		(heartTarget.glowLightIntensity - heartState.glowLightIntensity) *
		LERP_SPEED;
	heartState.bloodSpeed +=
		(heartTarget.bloodSpeed - heartState.bloodSpeed) * LERP_SPEED;
	heartState.bloodIntensity +=
		(heartTarget.bloodIntensity - heartState.bloodIntensity) * LERP_SPEED;
	heartState.fogDensity +=
		(heartTarget.fogDensity - heartState.fogDensity) * (LERP_SPEED * 0.5); // Slightly slower transition for fog
	...
	if (window._heartGroup) {
		...
		if (heartState.vibration > 0.0005) {
			...
			const vibX =
				Math.sin(time * 47.3) * Math.cos(time * 31.7) * heartState.vibration;
			const vibY =
				Math.sin(time * 53.1) * Math.cos(time * 29.3) * heartState.vibration;
