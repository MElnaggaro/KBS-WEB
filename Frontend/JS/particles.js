	...
	class Particle {
		...
		update(time) {
			...
			if (dist < CONFIG.mouseRepelRadius) {
				...
				const force =
					((dist - CONFIG.mouseRepelRadius) /
						(CONFIG.mouseAttractRadius - CONFIG.mouseRepelRadius)) *
					0.003;
				const angle = Math.atan2(dy, dx);
			...
			// Pulse
			this.opacity +=
				Math.sin(time * this.pulseSpeed + this.pulseOffset) * 0.05;
		...
		for (let i = 0; i < particles.length; i++) {
			for (let j = i + 1; j < particles.length; j++) {
				...
				if (dist < CONFIG.connectionDistance) {
					const opacity =
						(1 - dist / CONFIG.connectionDistance) * CONFIG.lineOpacity;
					...
					const mouseBoost =
						mouseDist < CONFIG.mouseAttractRadius
