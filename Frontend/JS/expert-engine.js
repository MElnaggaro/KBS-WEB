...
function computeRiskProfile(background) {
	...
	if (background.hypertension) {
		riskFactors.push("Hypertension");
		riskScore += 2;
	}
	if (background.diabetes) {
		riskFactors.push("Diabetes");
		riskScore += 2;
	}
	if (background.heart_disease) {
		riskFactors.push("Prior Heart Disease");
		riskScore += 3;
	}
	if (background.obesity) {
		riskFactors.push("Obesity");
		riskScore += 1;
	}
	if (background.smoking) {
		riskFactors.push("Smoking");
		riskScore += 2;
	}
	if (background.family_history) {
		riskFactors.push("Family History");
		riskScore += 1;
	}
	if (background.age >= 65) {
		riskFactors.push("Age ≥ 65");
		riskScore += 2;
	} else if (background.age >= 50) {
...
const URGENCY = {
	...
	MODERATE: {
		...
		label: "MODERATE",
		color: "moderate",
		action: "Visit a doctor within a few days",
	},
	LOW: {
		level: 1,
		label: "LOW",
...
const RULES = [
...

	{
		...
		condition: (s, v, b, f) => s.chest_pain && f.hr_status === "fast",
		result: {
...

	{
		...
		condition: (s, v, b, f) =>
			f.hb_status === "critical" &&
			(s.shortness_of_breath || s.fatigue) &&
			f.hr_status === "fast",
		result: {
...

	{
		...
		condition: (s, v, b, f) =>
			f.bp_status === "high" &&
			v.bp_systolic >= 180 &&
			(s.chest_pain || s.shortness_of_breath),
		result: {
...

	{
		...
		condition: (s, v, b, f) => f.hb_status === "low" && s.fatigue,
		result: {
...

	{
		...
		condition: (s, v, b, f) =>
			f.hr_status === "slow" &&
			(s.fatigue || s.shortness_of_breath || s.low_activity),
		result: {
...
function buildBaselineResult(
	...
	let explanation =
		"No clinical rules were triggered based on the provided data. ";
	...
	explanation +=
		"All vital signs appear within acceptable clinical ranges for this assessment. Continue standard health monitoring and attend routine check-ups.";
...
function buildVitalsSummary(vitals, facts) {
	return [
		{
			name: "Blood Pressure",
			value: `${vitals.bp_systolic} mmHg (systolic)`,
			status: facts.bp_status,
			label: facts.bp_label,
		},
		{
			name: "Heart Rate",
			value: `${vitals.heart_rate} bpm`,
			status:
				facts.hr_status === "fast" || facts.hr_status === "slow"
					? facts.hr_status === "fast"
						? "high"
						: "low"
					: "normal",
			label: facts.hr_label,
		},
		{
			name: "Respiratory Rate",
			value: `${vitals.respiratory_rate}/min`,
			status: facts.rr_status,
			label: facts.rr_label,
		},
		{
			name: "Oxygen Saturation",
			value: `${vitals.oxygen_saturation}%`,
			status: facts.o2_status === "critical" ? "critical" : facts.o2_status,
			label: facts.o2_label,
		},
		{
			name: "Hemoglobin",
			value: `${vitals.hemoglobin} g/dL`,
			status: facts.hb_status === "critical" ? "critical" : facts.hb_status,
			label: facts.hb_label,
		},
	];
}
